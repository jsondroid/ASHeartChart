...
import js.com.android.asheartchart.R;


/**
 * 显示心电的控件
 */
public class HeartView2 extends View {
    // 数据的最大值
    private int Max;
    // 数据的最小值
    private int Min;
    ...
    public HeartView2(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // 数据采集频率，默认100个点一秒钟
        hz = typedArray.getInteger(R.styleable.HeartView_heart_hz, 255);
        ...
        // 播放速度的控制
        heart_speed = typedArray.getFloat(R.styleable.HeartView_heart_speed, 0.3f);
        typedArray.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < showDatas.length; i++) {
            ...
            if (xCounts[i] > 0 && xCounts[i] % 100 == 0) {
                canvas.drawText(String.valueOf((float) (xCounts[i] / hz)) + "s", x, viewHeight - 40, paint);
            }
        }
